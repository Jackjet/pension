    ...
    <el-main>
                ...
                step: '00:15',
                    ...
                    <el-form-item :key='item' v-if="index.type === 'select'">
                        <el-select :placeholder='index.placeholder'
                                   ...
                                   size='medium'>
                            <el-option v-for='(dates, index) in index.option' :disabled="dates.disabled"
                                       :label='dates.label' :value="dates.value" :key="index"></el-option>
                        </el-select>
                    </el-form-item>
                </template>
        ...
        <el-col :span='24' style='text-align: left; padding: 0 2px;'>
            <template v-for='(item, index) in tableForm.item'>
                <el-button :key='index' type='primary' icon='el-icon-plus' plain size='mini'
                           v-if="item === 'new' || item.type === 'new'"
                           @click.native.prevent="item.method !==  undefined ? item.method() : ''">新增
                </el-button>
                <el-button :key='index' type='primary' icon="el-icon-delete" plain size='mini'
                           v-if="item === 'delete' || item.type === 'delete'"
                           @click.native.prevent="item.method !==  undefined ? item.method(deleteList) : ''">批量删除
                </el-button>
                <el-button :key='index' type='primary' icon='el-icon-document-copy' plain size='mini'
                           v-if="item === 'import' || item.type === 'import'"
                           @click.native.prevent="item.method !==  undefined ? item.method() : ''">导入表格
                </el-button>
                <el-button :key='index' type='primary' icon='el-icon-folder-opened' plain size='mini'
                           v-if="item === 'export' || item.type === 'export'"
                           @click.native.prevent="item.method !==  undefined ? item.method() : ''">导出表格
                </el-button>
            </template>
        </el-col>
        ...
        <el-col :span='24' v-else-if='tableForm.list'>
            <el-table
                    ...
                    empty-text='暂无数据'
                    :max-height="tableForm.list.maxHeight"
                    class='el_tab_alage'
                    :default-sort="{
          prop: 'createTime',
          order: 'descending'
                    ...
                    :cell-style="tableForm.list.styleMethod !== undefined ? tableForm.list.styleMethod.method : cellStyle"
                ...
                <el-table-column label="序号" type="index" width="50" align="center"
                                 v-if="tableForm.list.type === 'index'"></el-table-column>
                <el-table-column type="selection" width="50" align="center" v-if="isSelection"></el-table-column>
                <el-table-column type="expand" v-if="tableForm.list.detail">
                    <template slot-scope="scope">
                        <el-form label-position="left" inline class="demo-table-expand">
                            <el-form-item :label="indexs.label" v-for="(indexs, items) in tableForm.list.header"
                                          :key='items' v-show="indexs.expand">
                        ...
                        <template slot-scope="scope">
                            <template v-if="index.type === 'image'">
                                <el-image
                                    ...
                                    <div slot="error" class="image-slot" v-if="index.error==='person'">
                ...
                <el-table-column fit align='center'
                    ...
                    <template slot-scope="scope">
                        <el-button
                                ...
                                :icon="value.icon"
                                :key='item'
                                @click.native.prevent="value.method !== undefined ? value.method(scope.row, scope) : ''">
                            {{value.label}}
                        </el-button>
                    </template>
                </el-table-column>
