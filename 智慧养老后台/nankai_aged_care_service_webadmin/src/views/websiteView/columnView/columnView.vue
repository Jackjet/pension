  ...
  <el-main>
    ...
    <dialog-form :isShow="isShow" :isBtn="true" :title="title" @closeDialog="closeDialog" @saveDialog="saveDialog">
      <el-form slot="form" ref="form" :model="form" :rules="rules">
        <el-form-item label="栏目id" prop="id" label-width="108px">
          <el-input
            size="medium"
            autocomplete="off"
            v-model="form.id"
        ...
        <el-form-item label="栏目名称" prop="name" label-width="108px">
          <el-input
            size="medium"
            autocomplete="off"
            v-model="form.name"
        ...
        <el-form-item label="是否维护文章id" style="text-align: left;">
          <el-radio v-model="form.isId" label="1">是</el-radio>
          <el-radio v-model="form.isId" label="0">否</el-radio>
        </el-form-item>
      </el-form>
    </dialog-form>
  </el-main>
  ...
  methods: {
    ...
    saveDialog () {
      ...
      this.$refs['form'].validate(async (valid) => {
        if (valid) {
          try {
            let url = that.api.category.insert
            let obj = {
              id: that.form.id,
              name: that.form.name,
              isId: that.form.isId
    ...
    async edit (row) {
      ...
      try {
        ...
        if (response.code === 1) {
          ...
          that.form.isId = response.data.isId
          return false
        }
        that.$message.error(response.msg)
      } catch (even) {
    ...
    delete (row) {
      ...
      that.$confirm('您确定要删除么?', '提示', {
        ...
        let url = this.api.category.deleteById + '?id=' + row.id
