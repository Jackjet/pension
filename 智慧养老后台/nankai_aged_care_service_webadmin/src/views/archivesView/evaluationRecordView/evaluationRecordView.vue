<template>
  <el-main>
      ...
      <el-form :inline="true" class='el-InputForm'>
        ...
        <el-form-item style="width: 160px;">
          <el-select clearable size="medium" v-model="SearchItem.assessLevel" placeholder="请选择评估等级">
            <el-option v-for="item in assessLevel" :key="item.id" :label="item.name" :value="item.name">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item style="width: 160px;">
          <el-select clearable size="medium" v-model="SearchItem.assessInfo" placeholder="请选择评估信息">
            <el-option v-for="item in assessInfo" :key="item.id" :label="item.name" :value="item.name">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item style="width: 160px;">
        ...
        <el-form-item style="width: 330px;">
          <el-date-picker v-model="valueTime" size="medium" type="daterange" value-format="yyyy-MM-dd" range-separator="至" start-placeholder="操作时间" end-placeholder="结束时间">
          </el-date-picker>
        </el-form-item>
        <el-form-item>
    ...
    <el-col :span="24">
      <!-- 表格 -->
      <tableForm ref="trigger" :table-form='table' @getList='getList' @pageChange='pageChange'>
      </tableForm>
    </el-col>
  </el-main>
</template>
...
export default {
  ...
  methods: {
    ...
    async getList() {
      const that = this
      try {
        let findUrl = this.api.assessRecord.findAll
        ...
        response.data.content.forEach(function (item, index) {
          ...
          item.assessMoney = (Number(item.assessMoney) / 100).toFixed(2)
        })
