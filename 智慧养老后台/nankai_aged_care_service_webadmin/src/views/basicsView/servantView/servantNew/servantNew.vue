  ...
  <el-main>
    <el-form ref="form" :model="form" label-width="220px" :rules="rules">
      <el-form-item label="头像" prop="icon">
        <el-image :preview-src-list="[icon_show]" :src="icon_show" style="height: 100px;width: 100px">
          <div slot="error" class="image-slot">
            <el-image :lazy="true" fit="fill" :src="require('../../../../assets/image/error.png')" :preview-src-list="[require('../../../../assets/image/error.png')]">
            </el-image>
          </div>
        </el-image>
      </el-form-item>
      <el-form-item label="所属服务商" prop="providerId" v-if="ui.providerIdAvailabe === false">
        <el-select v-model="form.providerId" style="width: 280px" placeholder="请选择所属服务商">
          <el-option v-for="item in ui.providerIdSections" :key="item.value" :label="item.label" :value="item.value">
          </el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="所属服务商" prop="providerName" v-if="ui.providerIdAvailabe === true">
        <el-input v-model="form.providerName" :disabled="true"></el-input>
      </el-form-item>
      ...
      <el-form-item label="联系电话" prop="phone">
        <el-input v-model.number="form.phone" placeholder="请输入联系电话"></el-input>
      </el-form-item>
      ...
      <el-form-item label="身份证正面照" prop="idCard1">
        <el-image v-if="form.idCard1" style="width: 100px; height: 100px" :src="form.idCard1" :preview-src-list="[form.idCard1]">
          <div slot="error" class="image-slot">
            <el-image :lazy="true" fit="fill" :src="require('../../../../assets/image/error.png')" :preview-src-list="[require('../../../../assets/image/error.png')]">
            </el-image>
          </div>
        ...
        <div style="position: relative;">
          <el-upload class="upload-demo" accept='.jpg,.jpeg,.png,.gif,.bmp,.JPG,.JPEG,.PBG,.GIF,.BMP' :action='fileUrl' :headers='header' :show-file-list='false' :before-upload='beforeUpload' :on-success='handleChangeidCard1'>
      ...
      <el-form-item label="身份证背面照" prop="idCard2">
        <el-image v-if="form.idCard2" style="width: 100px; height: 100px" :src="form.idCard2" :preview-src-list="[form.idCard2]">
          <div slot="error" class="image-slot">
            <el-image :lazy="true" fit="fill" :src="require('../../../../assets/image/error.png')" :preview-src-list="[require('../../../../assets/image/error.png')]">
            </el-image>
          </div>
        ...
        <div style="position: relative;">
          <el-upload class="upload-demo" accept='.jpg,.jpeg,.png,.gif,.bmp,.JPG,.JPEG,.PBG,.GIF,.BMP' :action='fileUrl' :headers='header' :show-file-list='false' :before-upload='beforeUpload' :on-success='handleChangeidCard2'>
            <el-button size="small" type="primary">上传<i class="el-icon-upload el-icon--right"></i></el-button>
          </el-upload>
        </div>
      </el-form-item>
      ...
      <el-form-item label="现居住地址" prop="address">
        <el-input v-model="form.address" type="textarea" :rows="3" placeholder="请输入现居住地址"></el-input>
      </el-form-item>
      <el-form-item>
...
export default {
  data() {
    ...
    var checkProviderId = (rule, value, callback) => {
      if (this.form.providerId === '') {
        callback(new Error('服务商不可为空'))
      } else {
        callback()
      }
    }
    ...
    return {
      ...
      icon_show: '',
      rules: {
        providerId: [{ required: true, validator: checkProviderId, trigger: 'blur' }],
  ...
  methods: {
    ...
    async getInfo() {
      ...
      if (obj.id.length > 0) {
        ...
        that.ui.providerIdAvailabe = true
        if (response.code === 1) {
          that.form = response.data
          that.icon_show = response.data.icon ? process.env.VUE_APP_FILE + response.data.icon : that.errorImg
        ...
        that.form.sex = '1'
        that.form.icon_show = that.errorImg
        that.getProviderSection()
      }
    },
    async getProviderSection() {
      let that = this
      try {
        let obj = {
          size: 10000,
        ...
        let url = that.api.provider.findAll
        const response = await this.request.dataGet(that, url, obj)
        if (response.code === 1) {
          // that.$message.success(response.msg)
          that.ui.providerIdSections = []
          response.data.content.forEach(function (data) {
            let single = {
              label: data.name,
              value: data.id
            }
            that.ui.providerIdSections.push(single)
          })
    ...
    async submit() {
      ...
      try {
        ...
        if (obj.id.length > 0) {
          ...
          let obj = that.form
          obj.providerName = that.getLabelByValue(obj.providerId, that.ui.providerIdSections)
    ...
    async uploadPic(file, id) {
      const that = this
      try {
        let url = this.api.resourceLib.upload
        let obj = {
          'file': file
        }
        const response = await this.request.dataPost(that, url, obj)
        let urlPic = response.data.fileLongPath
        // console.log(urlPic)
        if (response.code === 1) {
          // that.$message.success(response.msg)
        } else {
