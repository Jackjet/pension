    ...
    async getList (target) {
      ...
      try {
        ...
        that.table.list.data.forEach(function (data) {
          data.sex_show = that.getLabelByValue(data.sex, that.sections.sex)
          data.icon_show = data.icon ? process.env.VUE_APP_FILE + data.icon : that.errorImg
        })
