  ...
  <el-main>
    <el-col :span="24">
      <el-form :inline="true" class='el-InputForm'>
        ...
        <el-form-item>
          <el-select clearable size="medium" v-model="SearchItem.communityId" placeholder="请选择所属社区" @change="community">
            <el-option
        ...
        <el-form-item>
          <el-select clearable size="medium" v-model="SearchItem.id" placeholder="请选择网格名称">
            <el-option
    ...
    <dialog-form :isShow="isShow" :isBtn="true" :title="title" @closeDialog="closeDialog" @saveDialog="saveDialog">
      <el-form slot="form" ref="form" :model="form" :rules="rules">
        <el-form-item label="所属街道" prop="streetId" label-width="108px" style="text-align:left">
          <el-select v-model="form.streetId" :disabled="id ? true : false" placeholder="请选择所属街道" @change="street">
            <el-option
              v-for="item in streetList"
              :key="item.id"
              :label="item.name"
        ...
        <el-form-item label="所属社区" prop="communityId" label-width="108px" style="text-align:left">
          <el-select v-model="form.communityId" :disabled="id ? true : false" placeholder="请选择所属社区">
            <el-option
              v-for="item in communityList"
              :key="item.id"
              :label="item.name"
        ...
        <el-form-item label="网格名称" prop="name" label-width="108px">
          <el-input
            size="medium"
            autocomplete="off"
            v-model="form.name"
  ...
  data () {
    return {
      ...
      rules: {
        ...
        name: [
          { required: true, message: '请输入网格名称', trigger: ['blur', 'change'] }
        ],
        linkName: [
  ...
  methods: {
    ...
    async getGridList (val) {
      ...
      try {
        ...
        const response = await this.request.dataGet(that, url, obj)
        that.gridList = response.data.content
      } catch (even) {
        that.$message.error('数据获取失败')
      }
    },
    ...
    saveDialog () {
      ...
      this.$refs['form'].validate(async (valid) => {
        if (valid) {
          try {
            let url = that.api.grid.insert
            let obj = {
              streetId: that.form.streetId,
              communityId: that.form.communityId,
              name: that.form.name,
    ...
    async edit (row) {
      ...
      this.getCommunityList()
      try {
        let findUrl = that.api.grid.findById
    ...
    async getList () {
      const that = this
      try {
        let findUrl = this.api.grid.findAll
