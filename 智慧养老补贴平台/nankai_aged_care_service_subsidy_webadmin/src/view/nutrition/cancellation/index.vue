<template>
  ...
  <div class="business-apply-page">
    ...
    <basic-information ref="obtainSonData" :details="details" @formValue="formValue" @registerAddress="registerAddress" :type="2" bType="2" bsType="百岁">
        ...
        <el-row :gutter="20">
          ...
          <!-- <el-col :span="24">
            <el-form-item label="注销原因" prop="cancelRemark">
              <el-input type="textarea" :rows="2" placeholder="请输入内容" v-model="cancellation.cancelRemark">
              </el-input>
            </el-form-item>
          </el-col> -->
        </el-row>
...
import { nutritionCancel, nutritionRecancel } from "@/api/nutrition/index.js";
...
export default {
  ...
  data() {
    return {
      ...
      rulesCancellation: {
        // cancelRemark: [{ required: true, message: '请选择注销原因', trigger: 'change' }],
  ...
  methods: {
    initStatus() {
      const that = this
      let data = getDate()
      findProcess({ businessType: 21, startTime: data }).then(res => {
        if (res.data.code === 1) {
      ...
      if (this.isSubmit.childrenForm && this.isSubmit.formSlot && this.isSubmit.formFile) {
        ...
        this.$confirm('温馨提示：请您仔细核实录入数据，申请过程中将无法修正！', '立即申请', {
          ...
          if (this.$route.query.idCard) {
            ...
            nutritionRecancel(this.submitObject).then(res => {
              if (res.data.code === 1) {
            ...
            nutritionCancel(this.submitObject).then(res => {
              if (res.data.code === 1) {
                this.$alert('温馨提示：您好，您提交的申请已转交至下一环节，请您耐心等候。', '立即申请', {
                  confirmButtonText: '确定',
                  callback: () => {
