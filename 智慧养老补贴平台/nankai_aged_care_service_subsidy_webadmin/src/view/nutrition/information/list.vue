<template>
  ...
  <div class="page-box">
    <div class="query-search">
      <el-form :inline="true" :model="formInline" class="demo-form-inline" label-width="120px">
        ...
        <el-form-item style="margin-left:10px">
          ...
          <!-- <el-button v-if="type==='2'" size="medium" type="primary" @click="batch">查看百岁老人营养补贴通知单</el-button> -->
          <el-button v-if="type==='3'" size="medium" type="primary" @click="batch">发放营养补贴</el-button>
        </el-form-item>
      </el-form>
    </div>
    ...
    <el-dialog title="上传街道签字确认表" :visible.sync="dialogVisible1" width="30%">
      <el-upload ref='upload' :action="actionUrl" :headers="headers" :on-change="handleChange" :on-remove="handleRemove" :file-list="fileList1">
...
import { subsidyFindAllByUser, batchInsert, livingProof, streetConfirm } from "@/api/theElderly/list.js";
...
export default {
  ...
  methods: {
    ...
    livingUpload() {
      livingProof({ id: this.livingRow.id, aliveFile: JSON.stringify(this.resultForm.jointAuditFile), businessType: 2 }).then((res) => {
        if (res.data.code === 1) {
    ...
    batch() {
      ...
      batchInsert({ subsidyIds: this.chooseId }).then((res) => {
        if (res.data.code === 1) {
          this.$message.success('当前月百岁营养补贴已成功发放！');
        } else {
          this.$message.error(res.data.msg);
        }
      })
    ...
    outExcel() {
      ...
      outExcel('请确认是否导出列表信息吗?', "/subsidy/webadmin/subsidyGrant/export", data, '百岁老人营养补贴信息')
    },
    toDetaile(row) {
      this.$router.push({
