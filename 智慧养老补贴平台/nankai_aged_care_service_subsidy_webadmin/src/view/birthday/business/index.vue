<template>
  ...
  <div class="business-apply-page">
    ...
    <basic-information ref="obtainSonData" @formValue="formValue" @registerAddress="registerAddress" @cardTypeChange="cardTypeChange" :type="type" :bType="bType">
    ...
    <div class="enclosure-file">
      ...
      <el-form ref="formFile" :model="fileJson" :rules="formFileRules" label-width="180px">
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <el-form-item label="存折/银行卡信息" :prop="cardType===2?'cardFile':''">
          ...
          <el-col :span="12">
            <el-form-item label="九十岁健在证明" prop="aliveFile">
...
import { birthdayApply, birthdayReapply } from "@/api/birthday/index.js";
...
export default {
  ...
  data() {
    return {
      ...
      formFileRules: {
        ...
        aliveFile: [{ required: true, message: '请上传九十岁建在证明', trigger: 'change' }],
      },
      fileJson: {
        applicationFile: [],//申请表
  ...
  methods: {
    initStatus() {
      const that = this
      let data = getDate()
      findProcess({ businessType: 3, startTime: data }).then(res => {
        if (res.data.code === 1) {
    ...
    onSubmit(formFile) {
      ...
      if (this.isSubmit.childrenForm && this.isSubmit.formSlot && this.isSubmit.formFile) {
        if (ages(this.submitObject.birthday) < 90) {
          this.$message('由于您年龄未满90周岁，暂不能申请九十岁营养补贴');
        } else {
          this.$confirm('温馨提示：请您仔细核实录入数据，申请过程中将无法修正！', '立即申请', {
            ...
            if (this.$route.query.idCard) {
              ...
              birthdayReapply(this.submitObject).then(res => {
                if (res.data.code === 1) {
              ...
              birthdayApply(this.submitObject).then(res => {
                if (res.data.code === 1) {
                  this.$alert('温馨提示：您好，您提交的申请已转交至下一环节，请您耐心等候。', '立即申请', {
                    confirmButtonText: '确定',
                    callback: () => {
