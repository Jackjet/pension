<template>
  <div class="home-page">
    ...
    <statistics-box v-if="update" title="90岁生日补贴业务" :currentPeriodData="currentPeriod" :handleList="handleList" :monthList='monthList'></statistics-box>
    <!-- <pie-chart v-if='len.length == 3' :echartsData="echartsData"></pie-chart> -->
  </div>
</template>

<script>
import { findGrantByMonth, findProcessByUser, findProcessByUserAndMonth } from "@/api/birthday";
...
export default {
      ...
      businessList: [
        {
          name: "90岁生日补贴业务申请",
          type: 6,
          router: "/birthday/business"
        },
        {
          name: "社区健在证明上报",
          router: "/birthday/information/list?type=1&olderType=100"
        },
        {
          name: "街道补贴上报",
          router: "/birthday/information/list?type=2&olderType=100"
        },
        {
          name: "民政补贴汇总",
          router: "/birthday/street/spendMoney"
        },
        {
          name: "街道补贴发放",
          router: "/birthday/information/list?type=3&olderType=100"
        },
        {
          name: "享受生日补贴老人档案",
          router: "/birthday/information/list?type=4&olderType=100"
        },
        {
          name: "我的消息",
          router: "/birthday/news/list"
        }
      ],
      handleList: [
        {
          routerPath: "/birthday/pending/list?olderType=100",
          num: 9,
          title: "待处理",
          processType: 0
        },
  ...
  created() {
    this.reload();
    sessionStorage.setItem("headerTitle", "90岁生日补贴")
  ...
  mounted() {
    ...
    // 当期发放信息
    findGrantByMonth({ businessType: 3 }).then(res => {
      if (res.data.code === 1) {
    ...
    findProcessByUserAndMonth({ streetId: this.streetId, communityId: this.communityId }).then(res => {
      if (res.data.code === 1) {
        ...
        this.monthList[2].num = res.data.data.remove ? res.data.data.remove : 0
        this.monthList[3].num = res.data.data.total ? res.data.data.total : 0
      } else {
        this.$message.error(res.data.msg);
      }
    })
