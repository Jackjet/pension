<template>
  <!-- 补贴业务申请（低保、低收入、优抚对象困难老人） -->
  <div class="business-apply-page">
    <business-process :dataList="applyList"></business-process>
    <basic-information ref="obtainSonData" @formValue="formValue" @registerAddress="registerAddress" @cardTypeChange="cardTypeChange" :type='type' bType="1">
...
import { homeAgedInsert, findProcess, homeAgedInsertEdit } from "@/api/theElderly/apply.js";
...
export default {
  ...
  methods: {
    initStatus() {
      const that = this
      let data = getDate()
      findProcess({ businessType: 1, startTime: data }).then(res => {
        if (res.data.code === 1) {
    ...
    onSubmit(formFile) {
      ...
      if (this.isSubmit.childrenForm && this.isSubmit.formSlot && this.isSubmit.formFile) {
        let type = this.submitObject.genreName;
        let message = "";
        let lrtye = "";//老人类型：多选项
        type.forEach(item => {
          if (item.name === '低保') {
            lrtye += "1";
            if (this.fileJson.lowLiveFile.length === 0) {
              message = "老人类型选中低保，最低生活保证金领取证必须上传。";
            }
          } else if (item.name === '低收入实物救助' || item.name === '低收入家庭救助') {
            lrtye += "2";
            if (this.fileJson.lowIncomeFile.length === 0) {
              message = "老人类型选中低收入实物救助或低收入家庭救助，城乡居民低收入救助证必须上传。";
            }
          } else if (item.name === '优抚') {
            lrtye += "3";
            if (this.fileJson.disabledSoldiersFile.length === 0) {
              message = "老人类型选中优抚，残疾军人证必须上传。";
            }
            if (this.fileJson.pensionFile.length === 0 && this.fileJson.veteransFile.length === 0) {
              message = "老人类型选中优抚，定期抚恤领取证或退役军人事务局认定凭证必须上传一项。";
            }
          }
        })
        if (lrtye === "12" || lrtye === "21") {//低保+低收入实物救助或低收入家庭救助
          if (this.fileJson.lowLiveFile.length === 0 && this.fileJson.lowIncomeFile.length === 0) {
            message = "老人类型同时选中低保、低收入实物救助、低收入家庭救助，最低生活保证金领取证、城乡居民低收入救助证，上传一项即可。";
          } else {
            message = "";
          }
        } else if (lrtye === "13" || lrtye === "31") {//低保+优抚
          if (this.fileJson.lowLiveFile.length === 0 && this.fileJson.disabledSoldiersFile.length === 0 && this.fileJson.pensionFile.length === 0 && this.fileJson.veteransFile.length === 0) {
            message = "老人类型同时选中低保、优抚，最低生活保证金领取证、残疾军人证，上传一项即可。";
          } else {
            message = "";
          }
          // if (this.fileJson.pensionFile.length === 0 && this.fileJson.veteransFile.length === 0) {
          ...
          //   message = "";
          // }
        } else if (lrtye === "23" || lrtye === "32") {//低收入实物救助或低收入家庭救助+优抚
          if (this.fileJson.lowIncomeFile.length === 0 && this.fileJson.disabledSoldiersFile.length === 0 && this.fileJson.pensionFile.length === 0 && this.fileJson.veteransFile.length === 0) {
            message = "老人类型同时选中优抚、低收入实物救助、低收入家庭救助，城乡居民低收入救助证、残疾军人证，上传一项即可。";
          } else {
            message = "";
          }
          // if (this.fileJson.pensionFile.length === 0 && this.fileJson.veteransFile.length === 0) {
          ...
          //   message = "";
          // }
        } else if (lrtye === "123" || lrtye === "321" || lrtye === "231" || lrtye === "312" || lrtye === "132" || lrtye === "213") {//低保+低收入实物救助或低收入家庭救助+优抚
          if (this.fileJson.lowLiveFile.length === 0 && this.fileJson.lowIncomeFile.length === 0 && this.fileJson.disabledSoldiersFile.length === 0 && this.fileJson.pensionFile.length === 0 && this.fileJson.veteransFile.length === 0) {
            message = "老人类型同时选中优抚、低保、低收入实物救助、低收入家庭救助，城乡居民低收入救助证、最低生活保证金领、残疾军人证，上传一项即可。";
          } else {
            message = "";
          }
          // if (this.fileJson.pensionFile.length === 0 && this.fileJson.veteransFile.length === 0) {
        ...
        if (message) {
          ...
          if (ages(this.submitObject.birthday) < 60) {
            this.$message('由于您年龄未满60周岁，暂不能申请居家养老补贴');
          } else {
            this.$confirm('温馨提示：请您仔细核实录入数据，申请过程中将无法修正！', '立即申请', {
              ...
              if (this.$route.query.idCard) {
                ...
                homeAgedInsertEdit(this.submitObject).then(res => {
                  if (res.data.code === 1) {
                ...
                homeAgedInsert(this.submitObject).then(res => {
                  if (res.data.code === 1) {
                    this.$alert('温馨提示：您好，您提交的申请已转交至下一环节，请您耐心等候。', '立即申请', {
                      confirmButtonText: '确定',
                      callback: () => {
